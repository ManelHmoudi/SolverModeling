...
alpha_r = 0.05   # energy coefficient (inventory → consumption)
...
ET      = {(l, t): 1.0    for l in clients for t in T}   # earliest arrival time
LT      = {(l, t): 3.0    for l in clients for t in T}   # latest arrival time
tau_min = 0.0
tau_max = 9_999.0
...
Cd   = 0.7      # aerodynamic drag coefficient
A_f  = 5.0      # frontal area (m²)
rho  = 1.2041   # air density (kg/m³)
a    = 0.0      # vehicle acceleration (assumed zero)
w    = 2500     # curb weight (kg)
...
e_co2 = 2.32    # CO2 conversion factor (kg CO2 / litre)

# -- Financial (BFR) ----------------------------------------------------------
BFR    = 3_000
DSO    = 30     # days sales outstanding (customer payment delay)
DPO    = 45     # days payable outstanding (supplier payment delay)
...
c1 = 2.0    # early-arrival penalty
c2 = 5.0    # late-arrival penalty
