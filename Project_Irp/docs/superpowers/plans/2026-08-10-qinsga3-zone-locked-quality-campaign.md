# QINSGA3 zone-locked decoder — quality campaign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and run the standard 3-seed quality campaign (HV/GD/IGD/Spacing, Mann-Whitney U vs. baseline and vs. NSGA-III) for the zone-locked decoder, following up on its positive continuity smoke-test result (`docs/superpowers/specs/2026-08-10-qinsga3-zone-locked-decoder-design.md`, Step 2 of Scope).

**Architecture:** `sensitivity/compare_zone_locked.py`, a standalone script mirroring `sensitivity/compare_damped_priority.py`'s exact structure (the project's established template for this kind of comparison): a local fork of `build_routes` wired to the zone-locked decoder, a private `IRPProblem` subclass, a `ProcessPoolExecutor`-parallelised `run_qinsga3` fork, and the shared comparison harness (shared ideal/nadir with the NSGA-III cache, `compute_pareto_metrics`, Mann-Whitney U). Reuses `sensitivity/zone_locked.py`'s already-tested `_zone_assignment`/`_make_zone_locked_decoder` (Task 1 of the prior plan) rather than re-deriving the zoning or construction logic.

**Tech Stack:** Python, numpy, pymoo, scipy (`mannwhitneyu`), `concurrent.futures.ProcessPoolExecutor`. No new dependencies.

## Global Constraints

- `Solvers/NSGA3/decoder.py` must never be modified on disk — the zone-locked decoder stays a local fork/closure, never monkey-patched into the shared module during the actual GA search (unlike the continuity smoke tests, which monkey-patched for a single decode call at a time — here evaluation is parallelised across worker processes, so each worker builds its own local decoder instance instead, avoiding any shared mutable module state).
- No change to `Solvers/QINSGA3/algorithm.py` — `run_qinsga3_zone_locked` is a full local fork, same convention as `run_qinsga3_damped_priority` in `compare_damped_priority.py`.
- Same standard protocol as every other remedy: 3 seeds first (`[42, 137, 271]`), full 300 generations, instance 100 clients, shared ideal/nadir with the NSGA-III cache, `repair_final_front=True` applied to both baseline and test arms (current production default since 2026-08-08 — omitting it from either arm would make the comparison invalid).
- Report the zone-locked decoder's unserved-client count (`sensitivity.zone_locked._unserved_count`) after the campaign — the design spec's Decision section explicitly flagged this as unchecked and a possible quality cost. Note the reported count is a final-front-repair-step lower bound only (see Task 1 Step 1's code comment) — the actual 300-generation search's own unserved-client rate isn't observable through this module-level counter once evaluation is parallelised across `ProcessPoolExecutor` workers, each with its own process memory.

Spec: `docs/superpowers/specs/2026-08-10-qinsga3-zone-locked-decoder-design.md` (Scope > Step 2)
Reference implementation this plan mirrors: `sensitivity/compare_damped_priority.py`

---

### Task 1: `sensitivity/compare_zone_locked.py` and a fast smoke run

**Files:**
- Create: `sensitivity/compare_zone_locked.py`

**Interfaces:**
- Consumes: `_zone_assignment`, `_make_zone_locked_decoder`, `_unserved_count` from `sensitivity/zone_locked.py`; `decode_chromosome`, `build_routes`, `_force_min_release`, `_clamp_to_integer_budget` from `Solvers/NSGA3/decoder.py`; `compute_f1..f4` from `Solvers/NSGA3/evaluator.py`; `compute_pareto_metrics` from `Solvers/NSGA3/metrics.py`; `IRPProblem` from `Solvers/NSGA3/problem.py`; `_encode_theta`, `_penalised_F`, `_normalise_F`, `_assign_ref_dirs`, `_select_guides`, `_supplement_from_archive`, `_migrate`, `_crowding_trim`, `_archive_update`, `_build_g_constraints` from `Solvers/QINSGA3/algorithm.py`; `QuantumPopulation` from `Solvers/QINSGA3/chromosome.py`; `_repair_route_result` from `Solvers/QINSGA3/repair.py`.
- Produces: `run_comparison(instance, seeds, max_gen, pop_size)` — prints the full HV/GD/IGD/Spacing/Mann-Whitney report to stdout, same format as `compare_damped_priority.py::run_comparison`. CLI entry point via `python -m sensitivity.compare_zone_locked`.

- [ ] **Step 1: Write `sensitivity/compare_zone_locked.py`**
...
- [ ] **Step 2: Fast smoke run (cost/correctness check, not the real campaign)**

Run: `python -m sensitivity.compare_zone_locked --gen 5 --seeds 42`
Expected: completes without error in well under a minute, prints a full report (front sizes, HV/GD/IGD/Spacing, Mann-Whitney, unserved-client count, timing) — values themselves are meaningless at `gen=5`, this step only confirms the pipeline runs end-to-end (multiprocessing workers pick up `zone_by_trucks` correctly, `IRPProblemZoneLocked`'s constraint count matches `_build_g_constraints`'s output length, `_repair_pareto_front_zone_locked` runs without shape errors).

- [ ] **Step 3: Commit**

```bash
git add sensitivity/compare_zone_locked.py
git commit -m "$(cat <<'EOF'
feat: quality campaign script for the QINSGA3 zone-locked decoder

Standard protocol (3 seeds, shared ideal/nadir, Mann-Whitney U vs.
baseline and vs. NSGA-III), mirroring compare_damped_priority.py's
structure. Follows up on the zone-locked decoder's positive continuity
smoke-test result -- first remedy in the whole campaign to restore a
real theta->route continuity gradient.
...
### Task 2: Run the 3-seed campaign and record the result

**Files:**
- Modify: `docs/superpowers/specs/2026-08-10-qinsga3-zone-locked-decoder-design.md` (append campaign Result)

- [ ] **Step 1: Run the real campaign**

Run: `python -m sensitivity.compare_zone_locked --seeds 42 137 271 --gen 300`
Expected: completes (budget several minutes per seed, consistent with other 300-gen campaigns in this project — e.g. `repair_final_front`'s ~211s/run), prints HV/GD/IGD/Spacing for NSGA-III/baseline/zone-locked, both Mann-Whitney comparisons, the unserved-client count, and per-run timings.

- [ ] **Step 2: Record the result**

Append a `## Result: quality campaign` section to
`docs/superpowers/specs/2026-08-10-qinsga3-zone-locked-decoder-design.md`
with the full printed report. Note explicitly that the printed
unserved-client count is a final-front-repair-step lower bound only (see
Global Constraints), not a full-search measurement — if it's non-zero even
at that lower bound, treat it as confirmed evidence the "no cross-zone
spillover" simplification does cost some served demand; if it's zero,
that's not proof the full search never left anyone unserved, just that
this particular check found none. State whether the continuity improvement
translated into HV/GD/IGD gains. If there's a positive or
promising-but-non-significant signal (matching this project's own decision
rule, e.g. Remède G's 3-seed p=0.10 floor before scaling), state whether
scaling to more seeds (7, then 20, same escalation every other remedy
followed) is worth it as a candidate next step — do not scale in this
task, only recommend.

- [ ] **Step 3: Commit**

```bash
git add docs/superpowers/specs/2026-08-10-qinsga3-zone-locked-decoder-design.md
git commit -m "$(cat <<'EOF'
docs: record zone-locked decoder 3-seed quality campaign result
...
No scaling past 3 seeds (7/20-seed escalation, if warranted by the result,
is a follow-up decision — same convention as every other remedy). No
production adoption regardless of outcome — ablation-only in `sensitivity/`
until/unless a scaled, significant result justifies promoting it (same
convention as `repair_final_front`'s adoption path).
