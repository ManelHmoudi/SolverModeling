"""Ring-structured guide selection test for QINSGA-III (8th remedy attempt,
after RQPSO dual-attractor rotation -- the 7th -- confirmed that ADDING a
second global attractor only compounds the contraction problem; see
Solvers/IRP_results_summary.md's "Septieme remede" section).

Context: all seven prior remedies either kept the SAME structural pattern
(every population member in a niche pulled toward ONE point, tuning that
point's frequency/target/magnitude, or adding a second such point) or
injected diversity at a different pipeline stage entirely (external reset,
measurement noise). None changed WHO a niche's single champion even is.

This 8th remedy is grounded in a single article section already used
elsewhere in this project for a different mechanism (§5, the reverted
DPQiEA operator): Tayarani-N & Akbarzadeh-T (2014), "Improvement of the
performance of the Quantum-inspired Evolutionary Algorithms: structures,
population, operators", Evolutionary Intelligence 7:219-239 -- §3, "New
structure for QiEA". The paper proposes 10 population topologies (Ring,
Star, Cellular, ...) where each q-individual only compares itself to its
LOCAL neighbours, not the whole population/niche -- Table 2 in the paper
found Ring specifically the best-performing structure for their Knapsack
problem (combinatorial, closest match to the IRP's own nature).

Solvers/QINSGA3/algorithm.py::_select_guides currently broadcasts the SAME
single niche champion (closest-to-reference-ray Pareto member) to EVERY
population member assigned to that niche -- exactly the mechanism the
diagnostic chapter found responsible for the chromosome-diversity collapse
(Var(X_norm) 13x lower than NSGA-III). _select_guides_ring replaces this:
each niche's population members are arranged in a ring (current array
order, recomputed fresh every generation -- no persistent per-individual
identity needed, unlike the pbest/eq.13 ports that broke on QINSGA3's
recombining population) and each member rotates toward the best of itself
and its two ring neighbours only. An instrumented check (60 generations,
real 100-client instance) confirmed this produces ~100 distinct guide
targets per generation on average, vs. ~43 (one per occupied niche) with
the current single-champion approach -- the mechanism demonstrably
decentralises the pull before any statistical test is run.

Already wired into production run_qinsga3 as use_ring_guides (default
False) -- no separate reimplementation needed.

Usage:
    python -m sensitivity.compare_ring_guides
    python -m sensitivity.compare_ring_guides --seeds 42 137 271
"""
...
def run_comparison(instance: str, seeds: list[int], max_gen: int, pop_size: int) -> None:
    ...
    print("=" * 92)
    print("  RING-STRUCTURED GUIDES — QINSGA-III (baseline single-champion vs NSGA-III cache)")
    print("=" * 92)
    ...
    baseline_lbl = "baseline (single champion)"
    test_lbl     = "Ring (test)"
    ...
    for seed in seeds:
        ...
        X, pareto_F, _ = run_qinsga3(
            sets_=sets_, params_=params_, ref_dirs=ref_dirs,
            pop_size=effective_pop, max_gen=max_gen, seed=seed, use_ring_guides=True,
        )
        elapsed = round(time.time() - t0, 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test des guides structures en anneau — QINSGA-III")
