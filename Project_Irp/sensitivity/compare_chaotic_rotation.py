"""Chaos-modulated rotation test for QINSGA-III (10th remedy attempt).

Context: every remedy tried so far (rotation frequency/target/magnitude
tuning, external reset, measurement noise, dual-attractor RQPSO, PSO
momentum, ring guides) either kept a smooth deterministic magnitude
schedule or added a smoothly-varying second attractor. None used a
genuinely CHAOTIC (as opposed to random or smoothly decaying) magnitude
term. Grounded in a single article read in full: Hu Feng-jun & Wu Bin
(2009), "Quantum Evolutionary Algorithm for Vehicle Routing Problem with
Simultaneous Delivery and Pickup", Joint 48th IEEE Conference on Decision
and Control / 28th Chinese Control Conference, eq. 15-19 -- the rotation
step's magnitude is an RMS distance to the niche's archive elites,
multiplicatively modulated by a per-individual logistic-map chaotic
sequence (mu>=4) instead of a monotonic alpha(g) decay. The logistic map's
stationary distribution is U-shaped (more time spent near 0 and 1 than near
0.5), producing bursty step sizes rather than a smooth schedule.

Three adaptations, all necessary (binary VRP encoding -> continuous IRP
domain, or numerical-stability guards), documented in full in the module
note above Solvers/QINSGA3/algorithm.py::_chaotic_rotate:
  - the paper's direction term depends on a binary bit of the best
    solution -- meaningless in continuous theta -- replaced by
    sign(guide_theta - theta), the same direction logic already used by
    every other rotation variant in this project;
  - the paper's own external best-solution archive B(t) maps directly onto
    QINSGA3's own external archive (no new elitism invented);
  - lambda's persistence across generations hits the same
    individual-identity mismatch as pbest/velocity (remedies C/D) --
    approximated per population slot, the same class of approximation
    already validated as non-degenerate for those two remedies.

Pre-campaign instrumented check (60 generations, real 100-client instance)
confirmed the mechanism is not degenerate: lambda shows real chaotic spread
in 100% of generations checked (mean per generation ~0.49, matching the
logistic map's known stationary mean), and the elite-RMS magnitude stays in
a reasonable, non-zero range throughout (mean 0.033-0.063 rad per
generation, 96-99% of individuals affected each generation).

Already wired into production run_qinsga3 as use_chaotic_rotation (default
False) -- no separate reimplementation needed.

Usage:
    python -m sensitivity.compare_chaotic_rotation
    python -m sensitivity.compare_chaotic_rotation --seeds 42 137 271
"""
...
def run_comparison(instance: str, seeds: list[int], max_gen: int, pop_size: int) -> None:
    ...
    print("=" * 92)
    print("  CHAOTIC ROTATION — QINSGA-III (baseline tanh vs NSGA-III cache)")
    print("=" * 92)
    ...
    for seed in seeds:
        ...
        X, pareto_F, _ = run_qinsga3(
            sets_=sets_, params_=params_, ref_dirs=ref_dirs,
            pop_size=effective_pop, max_gen=max_gen, seed=seed, use_chaotic_rotation=True,
        )
        elapsed = round(time.time() - t0, 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test de la rotation chaotique — QINSGA-III")
