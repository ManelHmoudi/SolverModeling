"""Final-front-only route repair test for QINSGA-III (remedy G, practical
variant) -- see docs/superpowers/specs/2026-08-04-qinsga3-route-repair-design.md.

Context: sensitivity/compare_route_repair.py measured remedy G's
every-generation variant (use_route_repair=True) at ~3.5x-15x slower than
baseline, even after optimising the 2-opt search itself (delta-cost
acceptance criterion, merged per-candidate path traversal, windowed
candidate search -- see Solvers/QINSGA3/repair.py). The slowdown is
structural: repairing every individual, every generation, is inherently
more expensive than a single evaluation, regardless of implementation
efficiency. The project owner's requirement was a solution that is BOTH
better AND not dramatically slower than NSGA-III -- the every-generation
variant cannot deliver that by construction.

This script tests the practical counterpart instead: repair_final_front=True
repairs ONLY the returned Pareto front, once, after the generational search
loop has already finished -- the loop's own runtime is completely
unaffected (identical to baseline), and only the reported front is
"polished" before being returned. Same Baldwinian repair mechanism
(2-opt, evaluated against real f1 cost) as use_route_repair, just applied
...
def run_comparison(instance: str, seeds: list[int], max_gen: int, pop_size: int) -> None:
    ...
    print("=" * 92)
    print("  FINAL-FRONT ROUTE REPAIR — QINSGA-III (baseline vs NSGA-III cache)")
    print("=" * 92)
    ...
    test_lbl     = "Final-front repair (test)"
    ...
    for seed in seeds:
        ...
        X, pareto_F, _ = run_qinsga3(
            sets_=sets_, params_=params_, ref_dirs=ref_dirs,
            pop_size=effective_pop, max_gen=max_gen, seed=seed, repair_final_front=True,
        )
        elapsed = round(time.time() - t0, 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test de la reparation du front final uniquement — QINSGA-III")
