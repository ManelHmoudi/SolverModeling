"""1-step-lookahead (regret-style) decoder test for QINSGA-III.

Context: `sensitivity/compare_soft_decoder.py` already tried fixing the
decoder's chaos (`_nearest_neighbour`'s strict argmin(score) flips choice
under tiny perturbations, and the construction never backtracks -- classic
greedy butterfly effect) by SOFTENING the choice with random (Boltzmann)
sampling. Rejected -- negative trend. Likely cause: softening trades input-
sensitivity for a NEW problem, evaluation noise (the same chromosome can now
decode to different routes on repeated calls), which is arguably worse for a
guided search than input-sensitivity alone.

This script tests a DETERMINISTIC alternative instead: at each construction
step, instead of picking the single best-scoring candidate (score =
distance/(0.5+priority), decoder.py:331), look at the K=2 best candidates
and simulate one further step for each -- pick whichever of the two leads to
the better 2-step cumulative score, not just the better 1-step score. No
randomness introduced anywhere; the same chromosome always decodes to the
same route. This is a "regret-style" / limited-lookahead construction
heuristic (reduces myopia without backtracking), a genuinely different
mechanism from softening.

Isolation from NSGA-III: same pattern as compare_soft_decoder.py -- a full
local fork of build_routes/_nearest_neighbour wired into a private
IRPProblem-like evaluation path used ONLY by this script's QI-NSGA-III test
variant. NSGA-III's reference numbers come from the existing chromosome
cache, decoded with the UNMODIFIED shared decoder -- completely unaffected.
Mandatory clients (floor>0, delivery-deadline-bound) keep the original
deterministic argmin, same reasoning as the soft-decoder script: lookahead
changes a routing PREFERENCE, not a hard deadline.

Cost warning (different risk profile from the repair-based remedies): this
decoder runs on EVERY chromosome evaluation (~200 pop x 300 gen x 2 = ~120k
decodes per run), not just on the ~40-65 solutions of a final front. K=2
lookahead costs roughly 2x the per-step candidate-scan work versus the
production decoder. ALWAYS smoke-test elapsed time on a short run before
committing to a full 300-generation campaign -- see `--gen 10` first.

Usage:
    python -m sensitivity.compare_lookahead_decoder --gen 10 --seeds 42   # cost smoke test
    python -m sensitivity.compare_lookahead_decoder --seeds 42 137 271 --gen 300 --k 2
"""
...
# ---------------------------------------------------------------------------
# Lookahead decoder (local fork of Solvers/NSGA3/decoder.py's construction step)
# ---------------------------------------------------------------------------

def _best_feasible_score(pending, current, load, current_time, cap, speed,
                          tau_max, d, s, O, floors, prios):
    """Lowest score among non-mandatory candidates feasible from this
    (current, load, current_time) state -- one step, no further lookahead.
    Returns None if no non-mandatory candidate is feasible (route likely
    ends here; contributes 0 to a lookahead sum, not a penalty -- ending a
    route is not inherently bad)."""
    best = None
    for l, q in pending.items():
        if (floors or {}).get(l, 0) > 0:
            continue  # mandatory handled separately, never part of lookahead
        if load + q > cap:
            continue
        dist = d[current, l]
...
def _nearest_neighbour_lookahead(qty_dict, trucks, t, d, v, s, Q, O, tau_max, floors, priorities, k_lookahead):
    """Same as Solvers/NSGA3/decoder.py::_nearest_neighbour, except the
    non-mandatory candidate is chosen by comparing the CUMULATIVE score over
    2 steps for the k_lookahead best 1-step candidates, instead of picking
    the 1-step argmin directly. Deterministic -- no randomness anywhere.
    Mandatory (floor>0) candidates keep the original 1-step argmin."""
    ...
    while pending and truck_idx < len(trucks):
        ...
        while True:
            ...
            candidates = []  # (l, score, q_effective)
            ...
            for l, q in pending.items():
                ...
                if is_mandatory:
                    ...
                    candidates.append((l, score, q_effective))

            if mand_best is not None:
                best, best_qty = mand_best, mand_qty
            elif candidates:
                candidates.sort(key=lambda c: c[1])
                top_k = candidates[:k_lookahead]
                if len(top_k) == 1:
                    best, _, best_qty = top_k[0]
                else:
                    best_total, best, best_qty = float("inf"), None, 0
                    for cand_l, cand_score, cand_q in top_k:
                        sim_load         = load + cand_q
                        sim_time         = current_time + s.get(current, 0.0) + d[current, cand_l] / speed
                        sim_pending      = {kk: vv for kk, vv in pending.items() if kk != cand_l}
                        next_score = _best_feasible_score(
                            sim_pending, cand_l, sim_load, sim_time, cap, speed,
                            tau_max, d, s, O, floors, prios,
                        )
                        total = cand_score + (next_score if next_score is not None else 0.0)
                        if total < best_total:
                            best_total, best, best_qty = total, cand_l, cand_q
            else:
                break
...
def build_routes_lookahead(quantities, sets_, params_, priorities, k_lookahead):
    """Local fork of Solvers/NSGA3/decoder.py::build_routes, calling
    _nearest_neighbour_lookahead instead of the original _nearest_neighbour.
    ...
    for t in T:
        ...
        for qty_group, trucks, floor_group in [
            ...
            if not qty_group or not trucks:
                continue
            r, tx, tf, ta, tassign = _nearest_neighbour_lookahead(
                qty_group, trucks, t, d, v, s, Q, O, tau_max, floor_group, priorities, k_lookahead,
            )
...
class IRPProblemLookahead(ElementwiseProblem):
    """Local fork of Solvers/NSGA3/problem.py::IRPProblem, using
    build_routes_lookahead instead of the shared decoder's build_routes."""

    def __init__(self, sets_, params_, k_lookahead):
        clients = sets_["clients"]
        T       = sets_["T"]
        q_lt    = params_["q_lt"]
        T_first = T[0]
        ...
        self.sets_, self.params_, self.k_lookahead = sets_, params_, k_lookahead

    def _evaluate(self, x, out, *args, **kwargs):
        quantities, priorities = decode_chromosome(x, self.sets_)
        route_result = build_routes_lookahead(quantities, self.sets_, self.params_, priorities, self.k_lookahead)
...
def _worker_init_lookahead(sets_, params_, k_lookahead):
    global _g_problem
    _g_problem = IRPProblemLookahead(sets_, params_, k_lookahead)


def _worker_eval_lookahead(x):
    out = {}
    _g_problem._evaluate(x, out)
    return out["F"], out["G"]


def run_qinsga3_lookahead_decoder(
    ...
    k_lookahead=DEFAULT_K,
    seed=42, rotation_type="tanh", callback=None,
):
    """Identical to production Solvers/QINSGA3/algorithm.py::run_qinsga3,
    except IRPProblem is replaced by IRPProblemLookahead (deterministic
    K-candidate lookahead decoder) for the search itself."""
    ...
    with ProcessPoolExecutor(
        max_workers=n_workers, initializer=_worker_init_lookahead,
        initargs=(sets_, params_, k_lookahead),
    ) as pool:

        def _eval_batch(X):
            results = list(pool.map(_worker_eval_lookahead, list(X), chunksize=chunksize))
...
def run_comparison(instance: str, seeds, max_gen: int, pop_size: int, k_lookahead: int) -> None:
    ...
    print("=" * 92)
    print(f"  DECODEUR A ANTICIPATION (k={k_lookahead}) — QINSGA-III (baseline vs NSGA-III cache)")
    print("=" * 92)
    ...
    raw = {"baseline (decodeur original)": [], f"anticipation k={k_lookahead} (test)": []}
    ...
    print(f"\n>>> anticipation k={k_lookahead} (test)")
    for seed in seeds:
        print(f"  seed={seed} ... ", end="", flush=True)
        t0 = time.time()
        X, pareto_F, _ = run_qinsga3_lookahead_decoder(
            sets_=sets_, params_=params_, ref_dirs=ref_dirs,
            pop_size=effective_pop, max_gen=max_gen, seed=seed, k_lookahead=k_lookahead,
        )
        elapsed = round(time.time() - t0, 1)
        raw[f"anticipation k={k_lookahead} (test)"].append((seed, pareto_F, elapsed))
        print(f"front={len(pareto_F) if pareto_F is not None else 0}  time={elapsed}s", flush=True)
    ...
    for lbl in ("baseline (decodeur original)", f"anticipation k={k_lookahead} (test)"):
    ...
    print(f"\n{'-'*92}")
    print(f"  Mann-Whitney U : baseline vs anticipation k={k_lookahead}")
    print(f"{'-'*92}")
    for metric in ("HV", "GD", "IGD", "Spacing"):
        a, b = groups["baseline (decodeur original)"][metric], groups[f"anticipation k={k_lookahead} (test)"][metric]
    ...
    print(f"\n{'-'*92}")
    print(f"  Mann-Whitney U : anticipation k={k_lookahead} vs NSGA-III")
    print(f"{'-'*92}")
    for metric in ("HV", "GD", "IGD", "Spacing"):
        a, b = groups[f"anticipation k={k_lookahead} (test)"][metric], groups["NSGA-III (reference)"][metric]
        if len(a) < 2 or len(b) < 2:
            print(f"  {metric:<8} : pas assez de runs valides")
            continue
        u, p = mannwhitneyu(a, b, alternative="two-sided")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test de decodeur a anticipation deterministe (regret-style) — QINSGA-III")
    ...
    run_comparison(args.instance, args.seeds, args.gen, args.pop, args.k)
