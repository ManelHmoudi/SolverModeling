...
It never looks at whether the rotation is actually helping. This is the one
mechanism with no equivalent in NSGA-III at all (NSGA-III has no "rotation
step"), so adapting it does not require touching NSGA-III's own code for a
fair comparison, unlike e.g. adaptive reference directions (A-NSGA-III,
Jain & Deb 2014 Part II) which would improve a mechanism the two algorithms
currently share.

Reference: this is a per-niche generalisation of Rechenberg's 1/5-success
rule (classic step-size self-adaptation in Evolution Strategies), applied to
the quantum rotation gate instead of a mutation step size -- in the same
spirit as the adaptive-rotation-angle quantum EA variants surveyed in
"Quantum rotation gate in quantum-inspired evolutionary algorithm: a review,
analysis and comparison study" (Gen & Cheng et al., Swarm and Evolutionary
Computation) and demonstrated in "Hybrid quantum genetic algorithm with
adaptive rotation angle for the 0-1 Knapsack problem" (Soft Computing, 2022).

Design: track, per reference-direction niche (stable across generations,
unlike individual population slots which get reshuffled by elitist
survival), whether its best (closest-to-ray) member's perpendicular distance
improved since the previous generation. If it improved, scale that niche's
rotation step UP (factor_up); if it stagnated or got worse, scale it DOWN
(factor_down), bounded to [scale_min, scale_max]. Every individual's rotation
step is alpha_base(g) * niche_scale[its niche] instead of alpha_base(g) alone.

This is deliberately NOT the same mechanism as the already-tried-and-reverted
per-niche "elitist guide" + "diversity preserving" operators (see
docs/superpowers/specs/2026-07-24-qinsga3-*-design.md, git commit 1b7e369):
those used stagnation to trigger a RESET of individuals' theta (found to
regress DTLZ3 and then have zero effect once "fixed"). This uses stagnation
only to modulate the ROTATION STEP SIZE -- no resets, no convergence-detection
threshold on individual thetas, just a coarser per-niche F-based signal.

Usage:
    python -m sensitivity.compare_adaptive_rotation
    python -m sensitivity.compare_adaptive_rotation --seeds 42 137 271 --gen 300
"""
...
def _niche_best_perp_dist(assoc: np.ndarray, F_norm: np.ndarray, ref_dirs: np.ndarray) -> dict:
    """Per-niche best (smallest) perpendicular distance to its reference ray."""
    ...
    for rd in np.unique(assoc):
        rd  = int(rd)
        idx = np.where(assoc == rd)[0]
        F_n = F_norm[idx]
        proj    = F_n @ ref_unit[rd]
        d_perp2 = np.maximum((F_n ** 2).sum(axis=1) - proj ** 2, 0.0)
        out[rd] = float(d_perp2.min())
    return out


def _update_niche_rotation_scale(
    prev_best:   dict,
    cur_best:    dict,
    scale:       dict,
    factor_up:   float = 1.1,
    factor_down: float = 0.9,
    scale_min:   float = 0.3,
    scale_max:   float = 2.0,
) -> dict:
    """Per-niche 1/5-success-rule-style step adaptation [Rechenberg;
    quantum-EA adaptive-rotation-angle variants]: scale UP a niche's rotation
    step if its best member got closer to the reference ray since last
    generation, scale DOWN if it stagnated or regressed.
    """
    new_scale = dict(scale)
    for rd, best in cur_best.items():
        cur = new_scale.get(rd, 1.0)
        prev = prev_best.get(rd)
        if prev is not None:
            if best < prev - 1e-12:
                cur = min(scale_max, cur * factor_up)
            else:
                cur = max(scale_min, cur * factor_down)
        new_scale[rd] = cur
    return new_scale


def run_qinsga3_adaptive_rotation(
    ...
    factor_up=1.1, factor_down=0.9, scale_min=0.3, scale_max=2.0,
    seed=42, rotation_type="tanh", callback=None,
):
    """Identical to production QINSGA3.algorithm.run_qinsga3 (elitist
    survival + X-space SBX/PM, rotation in theta-space), except the rotation
    step alpha is scaled per-niche based on whether that niche's best member
    improved since the previous generation, instead of depending only on the
    generation index.
    """
    ...
    with ProcessPoolExecutor(
        ...
        for gen in range(max_gen):
            ...
            # --- Adaptive rotation step: per-niche 1/5-rule-style scaling ---
            cur_best    = _niche_best_perp_dist(assoc, F_norm, ref_dirs)
            niche_scale = _update_niche_rotation_scale(
                niche_prev_best, cur_best, niche_scale,
                factor_up, factor_down, scale_min, scale_max,
            )
            niche_prev_best = cur_best

            alpha_base = alpha_min + (alpha_max - alpha_min) * (1.0 - gen / max_gen)
            scale_vec  = np.array([niche_scale.get(int(a), 1.0) for a in assoc])[:, None]
            alpha_vec  = alpha_base * scale_vec   # (pop_size, 1), broadcasts over genes
...
def run_comparison(instance: str, seeds: list[int], max_gen: int, pop_size: int) -> None:
    ...
    print("=" * 92)
    print("  ADAPTIVE ROTATION-ANGLE TEST — QINSGA-III (vs NSGA-III cache)")
    print("=" * 92)
    ...
    raw: dict[str, list] = {"baseline (prod. actuelle)": [], "adaptive_rotation (test)": []}
    ...
    print("\n>>> adaptive_rotation (pas de rotation adapte par niche, 1/5-rule)")
    for seed in seeds:
        print(f"  seed={seed} ... ", end="", flush=True)
        t0 = time.time()
        _, pareto_F, _ = run_qinsga3_adaptive_rotation(
            sets_=sets_, params_=params_, ref_dirs=ref_dirs,
            pop_size=effective_pop, max_gen=max_gen, seed=seed,
        )
        elapsed = round(time.time() - t0, 1)
        raw["adaptive_rotation (test)"].append((seed, pareto_F, elapsed))
        print(f"front={len(pareto_F) if pareto_F is not None else 0}  time={elapsed}s", flush=True)
    ...
    for lbl in ("baseline (prod. actuelle)", "adaptive_rotation (test)"):
    ...
    print(f"\n{'-'*92}")
    print("  RESULTATS (ideal/nadir global partage NSGA-III + baseline + adaptive_rotation)")
    print(f"{'-'*92}")
    ...
    print(f"\n{'-'*92}")
    print("  Mann-Whitney U : baseline vs adaptive_rotation")
    print(f"{'-'*92}")
    for metric in ("HV", "GD", "IGD", "Spacing"):
        a, b = groups["baseline (prod. actuelle)"][metric], groups["adaptive_rotation (test)"][metric]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test de rotation adaptative QINSGA-III")
