"""Measurement-noise strengthening test for QINSGA-III (6th remedy attempt,
after niche-recentring reset -- the 5th -- restored diversity but degraded
quality; see Solvers/IRP_results_summary.md's "Cinquieme remede" section).

Context: unlike all five prior remedies (which all acted BEFORE or AT the
theta level -- rotation frequency/target/magnitude, or an external reset of
theta itself), this one leaves theta untouched and targets a different,
never-before-tested step: the MEASUREMENT (theta -> x). Grounded in a single
verified source, read in full: Guzel, Yildirim Okay, Kok & Ozdemir (2022),
"QNSGA-II: A Quantum Computing-Inspired Approach to Multi-Objective
Optimization", IEEE ISNCC. That paper's QNSGA-II starts its ENTIRE population
from identical quantum chromosomes (alpha=beta for every qubit) and relies
solely on probabilistic measurement (xi = 0 if rand[0,1] < alpha_i^2 else 1)
for all population diversity -- explicitly stated: "The diversity in
QNSGA-II is formed in the measurement function."

QINSGA3 already has a measurement-noise term for exactly this purpose
(chromosome.py::measure, sigma = noise_scale * |sin(2*theta)| * (xu-xl),
Platel et al. 2009) but it is tuned to noise_scale=0.02 -- small enough that
its own docstring describes it as being there only "to prevent nearby theta
values from collapsing to identical integer routes after the IRP decoder
rounds to integers", a tie-breaker, not a real diversity source. Guzel et
al.'s design treats the measurement as THE diversity mechanism, not a
tie-breaker -- this remedy tests strengthening noise_scale enough to actually
carry that load, following that principle (the noise's functional FORM,
sigma proportional to |sin(2*theta)| peaking at theta=pi/4 -- exactly where
the population is measured to sit -- is unchanged; only its magnitude is
tested, since the paper's own mechanism has no continuous-domain magnitude
parameter to port directly -- binary Bernoulli sampling has no "how much"
knob, this is the necessary continuous-domain adaptation, calibrated
empirically like every other new operator parameter in this project).

Already wired into production run_qinsga3 as noise_scale (default 0.02,
unchanged) -- no separate reimplementation needed.

Usage:
    python -m sensitivity.compare_measurement_noise
    python -m sensitivity.compare_measurement_noise --seeds 42 137 271 --noise 0.15
"""
...
def run_comparison(instance: str, seeds: list[int], max_gen: int, pop_size: int, noise: float) -> None:
    ...
    print("=" * 92)
    print(f"  BRUIT DE MESURE RENFORCE (noise_scale={noise}) — QINSGA-III (baseline vs NSGA-III cache)")
    print("=" * 92)
    ...
    baseline_lbl = f"baseline (noise_scale={BASELINE_NOISE})"
    test_lbl     = f"noise_scale={noise} (test)"
    ...
    for seed in seeds:
        ...
        X, pareto_F, _ = run_qinsga3(
            sets_=sets_, params_=params_, ref_dirs=ref_dirs,
            pop_size=effective_pop, max_gen=max_gen, seed=seed, noise_scale=noise,
        )
        elapsed = round(time.time() - t0, 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test du bruit de mesure renforce — QINSGA-III")
    ...
    run_comparison(args.instance, args.seeds, args.gen, args.pop, args.noise)
